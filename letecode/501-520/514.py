...
class Solution:
    def findRotateSteps(self, ring: str, key: str) -> int:
        ...
        for cur in key:
            cur_targrt = d[cur]
            last = float('inf')
            for target in cur_targrt:
                min_v = float('inf')
                for pre in pre_list:
                    diff = abs(target - pre)
                    min_v = min(min_v, min(diff, length - diff) + dp[pre])
                dp[target] = min_v
                last = min(last, min_v)
            pre_list = cur_targrt
        return last + len(key)
...
if __name__ == '__main__':
    sol = Solution()
    print(sol.findRotateSteps("caotmcaataijjxi", "oatjiioicitatajtijciocjcaaxaaatmctxamacaamjjx"))
