...
class Solution:
    def exclusiveTime(self, n: int, logs):
        d = {('start', 'end'): 1, ('start', 'start'): 0, ('end', 'end'): 0, ('end', 'start'): -1}
        res = [0] * n
        for i, log in enumerate(logs):
            log = log.split(':')
            if i == 0:
                stack = [[int(log[0]), log[1], int(log[2])]]
                continue
            func = int(log[0])
            state = log[1]
            time = int(log[2])
            if state == 'end':
                res[func] += time - stack[-1][2] + d[(stack[-1][1], state)]
                stack.pop()
                if stack:
                    stack[-1][2] = time + 1
            else:
                if stack:
                    res[stack[-1][0]] += time - stack[-1][2] + d[(stack[-1][1], state)]
                    stack[-1][2] = time
                stack.append([func, state, time])
        return res
