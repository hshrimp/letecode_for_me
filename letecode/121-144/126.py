...
class Solution:
    def findLadders(self, beginWord: str, endWord: str, wordList):
        ...
        def track(li):
            ...
            for i in range(length):
                cur_word = li[-1][:i] + '*' + li[-1][i + 1:]
                for word in d[cur_word]:
                    if word not in li:
                        track(li + [word])
    ...
    def findLadders2(self, beginWord: str, endWord: str, wordList):
        ...
        while seq:
            ...
            if l >= max_len:
                continue
            for i in range(length):
                cur_w = word[:i] + '*' + word[i + 1:]
                if endWord in d[cur_w]:
                    if l + 1 == max_len:
                        res.append(li + [endWord])
                    elif l + 1 < max_len:
                        res = [li + [endWord]]
                        max_len = l + 1
                    continue
                for w in d[cur_w]:
                    if w not in visited or (w in visited and visited[w] >= l):
                        visited[w] = l
                        seq.append(li + [w])
        end2 = time.time()
        print('end1-start', end1 - start)
...
if __name__ == '__main__':
    beginWord = "sand"
    endWord = "acne"
    wordList = ["slit", "bunk", "wars", "ping", "viva", "wynn", "wows", "irks", "gang", "pool", "mock", "fort", "heel",
